...
# bank installments
class FE_bank_installment_trades:
	# transform
	def transform(self, X):
...
# trades over income
class FE_trade_to_income:
	def transform(self, X):
		#income and tradeline info
		# 32s 33s 34s 35s = max /total / utilization / average verified 12 months
		# ================================== trades over income ==============================================
		try:
...
# auto trades to income
class FE_auto_to_income:
	def transform(self, X):
...
# income to auto trades
class FE_income_to_auto:
	def transform(self, X):
...
# credit card over income
class FE_cc_to_income:
	def transform(self, X):
...
# bank installments to income
class FE_bi_to_income:
	def transform(self, X):
...
# income to bank installments
class FE_income_to_bi:
	def transform(self, X):
...
# finance revolver over income
class FE_fr_to_income:
	def transform(self, X):
...
# Home equity LOC to income
class FE_hr_to_income:
	def transform(self, X):
		#========================== HE LOC over income==================================================
		try:
...
# pfs inttype vs TU BK ----- need to do feature value replacer on fields that to get outputs line up with 0, 1 etc
class FE_pfs_vs_tu_bk:
	def transform(self, X):
		try:
			X['eng_inttype_times_linkf032'] = X['intType__app'] * X['LINKF032__tucvlink__bin']
		except:
			pass
...
# BK Credit Vision vs Factor Trust
class FE_bk_cv_vs_ft:
	def transform(self, X):
...
# additional LexisNexis address fields 
class FE_ln_2:
	def transform(self, X):
		# addr newest vs oldest
		try:
			X['eng_addr_new_vs_old'] = X['addrprevioustimenewest__ln'] / X['addrprevioustimeoldest__ln']
		except:
			pass
		# AVMV value 12mo over historic
		try:
			X['eng_AVMV_12mo_vs_old'] = X['addrcurrentavmvalue60month__ln'] / X['addrcurrentavmvalue__ln']
		except:
			pass
		# input AVMV value 12mo over historic
		try: 
			X['eng_input_AVMV_12mo_vs_old'] = X['addrinputavmvalue60month__ln'] / X['addrinputavmvalue__ln']
		except:
			pass 
		return X 
...
list_cols_eng_raw = ['fltAmountFinanced__app', 'fltApprovedPriceWholesale__app', 'jt03s__tuaccept',
					 ...
					 'shorttermloanrequest24month__ln', 'shorttermloanrequest__ln', 'addrprevioustimenewest__ln', 'addrprevioustimeoldest__ln',
					 'addrcurrentavmvalue60month__ln', 'addrcurrentavmvalue__ln', 'addrinputavmvalue60month__ln', 'addrinputavmvalue__ln']
