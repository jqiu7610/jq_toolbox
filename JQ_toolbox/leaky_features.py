...
class LeakyFeatures(TrainValidTestSplit):
	...
	def check_for_score_columns(self, list_cols):
		...
		return self
	# fit model
	def fit_model(self, df_train, df_test, list_cols_model, flt_prop_early_stopping=0.10, verbose=100, dict_monotone_constraints=None, int_n_iterations=1000, str_eval_metric='AUC', flt_learning_rate=None, list_class_weights=None, int_random_state=42):
		...
		# get int_best_iteration
		int_best_iteration = cls_model_inference.get_best_iteration()
		...
		# if doing regression
			...
			# if eval metric is MAE
			elif str_eval_metric == 'MAE':
				# eval metric
				flt_eval_metric = skm.mean_absolute_error(y_true=y_test, y_pred=y_hat)
		...
		dict_pipeline = {
			...
			'int_best_iteration': int_best_iteration,
		}
	...
	def get_feature_importance(self, df_test, list_non_numeric=None, str_filename='df_feat_imp.csv'):
		# get list_non_numeric and inference model
		try:
			list_non_numeric = self.dict_pipeline['list_non_numeric']
			cls_model_inference = self.dict_pipeline['model_inference']
		except AttributeError:
			cls_model_inference = self.cls_model_inference
		# pool data
		df_test_pooled = cb.Pool(
			df_test[cls_model_inference.feature_names_],
			df_test[self.str_target],
			list_non_numeric,
		)
		# get feature importance
		df_feat_imp = cls_model_inference.get_feature_importance(
			data=df_test_pooled,
		...
		# save to .csv
		df_feat_imp.to_csv(f'{self.str_dirname_output}/{str_filename}', index=False)
		# save to object
